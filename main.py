...
def main():
    ...
    class App(ttk.Frame):
        def __init__(self, master):
            ...
            self.img_frame = tk.Frame(self.input_frame, highlightbackground="black", highlightthickness=0,
                                      bg="darkgrey")
            ...
            self.pwr_ent = ttk.Combobox(self.param_frame, width=8)
            self.pwr_ent.bind('<<ComboboxSelected>>', self.fixed_or_variable)
            self.pwr_ent.bind('<KeyRelease>', self.fixed_or_variable)
            self.to_lab = tk.Label(self.param_frame, text="to")
            self.pwr_ent_2 = ttk.Combobox(self.param_frame, width=8)
            self.pwr_ent_2.bind('<<ComboboxSelected>>', self.fixed_or_variable)
            self.pwr_ent_2.bind('<KeyRelease>', self.fixed_or_variable)
            self.pwr_unit = ttk.Combobox(self.param_frame, textvariable=self.units, width=9)
            self.pwr_unit.bind('<<ComboboxSelected>>', self.set_info)
            self.desc_lab = tk.Label(self.param_frame, text="Power description:")
            self.desc_ent = tk.Entry(self.param_frame, width=23)
            ...
            self.bar_p_lab = tk.Label(self.ambient_controls, text="Barometric pressure:")
            self.bar_p_ent = tk.Entry(self.ambient_controls, width=20)
            self.bar_p_unit = tk.Label(self.ambient_controls, text="inches Hg")
            self.rh_pct_lab = tk.Label(self.ambient_controls, text="Relative humidity:")
            ...
            self.levels_lab = tk.Label(self.static_controls, text="Contour levels:")
            self.levels_ent = tk.Entry(self.static_controls, width=20)
            self.levels_unit = tk.Label(self.static_controls, text="dB")
            self.grids_lab = tk.Label(self.static_controls, text="Number of grids:")
            self.grids_ent = tk.Entry(self.static_controls, width=4)
            ...
            self.pwr_lab.grid(row=3, column=0, sticky="E")
            self.pwr_ent.grid(row=3, column=1, sticky="W")
            self.pwr_ent_2.grid(row=3, column=2, sticky="E")
            self.pwr_unit.grid(row=3, column=3, sticky="E")
            ...
            self.op_type_1.grid(row=1, column=1, sticky="W")
            self.op_type_2.grid(row=1, column=2, sticky="W")
            self.static_lab.grid(row=8, column=0, sticky="W")
        ...
        def populate_aircraft(self, op_type):
            df = fn.get_operations(type=int(op_type))
            self.ac_drp["values"] = pd.unique(df["aircraft"]).tolist()

        def toggle_static(self):  # Disable non-flyover/static entries
            ...
            if test_op == "2":
                ...
                self.to_lab['text'] = ''
                self.pwr_2.set("")
            self.populate_aircraft(test_op)
            self.set_info(None)
        ...
        def set_info(self, event):  # Get units and engine of selected aircraft
            try:
                ...
                if self.ac.get() != self.ac_mem.get():
                    print("AC Changed, updating units list")
                    self.units.set(units[0])
                elif all( [i == j for i, j in zip(units, self.units_mem.get())] ) == False:  # If the aircraft or units list changed, update the units.
                    print("Units list changed, updating units list")
                    self.units.set(units[0])
                ...
                print(self.units.get(), self.units_mem.get(), units)

                which_units = units.index(self.units.get()) + 1
                op = "FLIGHT" if self.op_type.get() == "1" else "STATIC"
                fixed_ops = ops[
                    (ops.aircraft == self.ac.get()) & (ops.operation_type == op) & (ops.interpolation == "FIXED")]
                fixed = fixed_ops["power_" + str(which_units)].tolist()
                self.pwr_ent["values"] = fixed
                self.pwr_ent_2["values"] = fixed
                self.pwr.set("")
                self.pwr_2.set("")
            except:
        ...
        def fixed_or_variable(self, event=None):
            ...
            if pwr in fixed_vals and pwr_2 is None:
                ...
                self.interp.set(2)
            elif pwr in fixed_vals and pwr_2 in fixed_vals:
                # pwr and pwr_2 in fixed vals. Interpolation should be fixed
                self.interp.set(2)
            else:
                # Interpolation should be variable
                self.interp.set(1)
        ...
        def make_plot(self, save_name=None):  # make a dataframe, call plotting function
            ...
            if i == "VARIABLE":
                self.correct_range(self.ac.get(),
                                   self.pwr.get())  # Correct to power setting within variable range if variable interpolation
            else:
                pass
            ...
            if self.op_type.get() == "1":  # Flyover plot
                ...
                # Try to make dataframes from o10 output. Catch error and warn about fixed/variable interpolation
                try:
                ...
                # Try to make dataframes from o11 output. Catch error and warn about fixed/variable interpolation
                try:
                ...
                try:
                    fig = nplot.plot_contour(df, aircraft=self.ac.get(), engine=self.eng.get(),
                                             ...
                                             n_grids=self.n_grids.get(),
                                             levels=[float(l.strip()) for l in self.levels.get().split(",")],
        ...
        def correct_range(self, ac, pwr, file="./data/operation_data.csv"):
            if self.interp.get() == 1:  # Only correct power settings with variable interpolation
                p = float(pwr)
                df = pd.read_csv(file)
                ...
                if p > rng[0] and p < rng[1]:  # if pwr is in range
                    ...
                    pass
            else:  # Pass if variable interpolation not available
                pass
